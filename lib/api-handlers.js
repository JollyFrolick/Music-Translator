...
export async function processStripeWebhookPayload(payload, signature) {
  const event = verifyStripeWebhookPayload(payload, signature, getStripeWebhookSecret());
  ...
  if (!isPaidCheckoutSession(session)) {
    console.warn("Stripe checkout session was not paid yet.");
    return { status: 200, body: { received: true, ignored: true } };
  }
  ...
  console.info("Stripe checkout upgraded a user to Premium.");
  return { status: 200, body: { received: true, premium: true } };
}
...
export async function handleStripeWebhook(request, response) {
  ...
  try {
    ...
    console.error(error);
    return sendJson(response, error.status || 500, { error: error.message || "Could not process Stripe webhook." });
  }
}
